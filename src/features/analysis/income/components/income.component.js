...
export const IncomeHelper = (records, endingMonth) => {
  const result = [];
  ...
  const temp = combineCatData(monthlyData(endingMonth, false, records)).sort(
    compare
  );
  ...
  for (var j = 0; j < label.length; j++) {
    ...
    for (var i = 0; i < 3; i++) {
      const tempHolder = combineCatData(
        monthlyData(yearMonthDecrement(endingMonth, i), false, records)
      );
      const amt =
